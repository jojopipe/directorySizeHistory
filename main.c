...
const char HOME_DIR[64] = "/home/joppo";
...
void crawl_init(char path[]) {
    ...
    *sizeSum = 0;
    ...
    char log_dir[1024];
    snprintf(log_dir, 1024, "%s/%s", HOME_DIR, "dirSizeLogs");

    int status;
    mode_t mode = 0000770; //owner and group can do rwx
    status = mkdir(log_dir, mode);
    if (status != 0) {
        printf("%s\n", "Could not create directory. It may already exist.");
    }
    char *name = basename(path);

    char log_file[4096];
    snprintf(log_file, 4096, "%s/%s.log", log_dir, name);
